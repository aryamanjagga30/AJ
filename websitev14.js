// WEBSITEV14 - Mobile Detection + Original Desktop Experience

// ===== MOBILE DETECTION =====
function isMobileDevice() {
  const userAgent = navigator.userAgent || navigator.vendor || window.opera;

  // Check screen size (anything under 1024px width or 768px height)
  ...
  // Exclude tablets in landscape mode with larger screens
...
if (IS_MOBILE) {
  // Show mobile view, hide desktop UI
  document.getElementById('mobile-view').classList.remove('hidden');
  document.getElementById('desktop-experience').classList.add('hidden');

  // Load simplified mobile scene
  initMobileScene();
} else {
  // Show desktop, hide mobile
  document.getElementById('mobile-view').classList.add('hidden');
  document.getElementById('desktop-experience').classList.remove('hidden');

  // Load full desktop experience
  initDesktopScene();
}

// ===== MOBILE SCENE (FIXED CAMERA - SCENE 2 POSITION) =====
function initMobileScene() {
  ...
  // FIXED CAMERA - Scene 2 position (close to avatar)
  ...
  console.log('📱 Mobile scene loaded');
}

// ===== DESKTOP SCENE (ORIGINAL V14) =====
function initDesktopScene() {
  ...
  function animate() {
    ...
    if (groundLight) {
      groundLight.intensity = 95 + Math.sin(time * 0.0005) * 5;
    }
...
console.log("🎬 V14 with mobile detection");